...
void ButtonManager::dispatchEvt(Button *b, Button::Event evt)
{
	...
	case Io::ButtonB:
		...
		if (Ui::instance.onButtonEvent(b, evt)) {
			break;
		}
		else if (UAV::instance1.onButtonEvent(b, evt)) {
			Haptic::startPattern(Haptic::SingleShort);
			break;
		}
		else if (UAV::instance2.onButtonEvent(b, evt)) {
			Haptic::startPattern(Haptic::SingleShort);
			break;
		}
		else if (UAV::instance3.onButtonEvent(b, evt)) {
			Haptic::startPattern(Haptic::SingleShort);
			break;
		}
		else if (b->id() == Io::ButtonRTL && evt == Button::LongHold) {
			UAV::instance1.forceRTL();
			UAV::instance2.forceRTL();
			UAV::instance3.forceRTL();
			Haptic::startPattern(Haptic::HeavyTriple);
		}
		break;
	case Io::ButtonPreset1:
		if (evt == Button::LongHoldRelease || evt == Button::HoldRelease) {
			if (UAV::instance1.isActive()) {
				UAV::instance1.setActive(false);
				UAV::instance2.setActive(true);
				Haptic::startPattern(Haptic::SingleShort);
			}
			else if (UAV::instance2.isActive()) {
				UAV::instance2.setActive(false);
				UAV::instance3.setActive(true);
				Haptic::startPattern(Haptic::SingleShort);
			}
			else if (UAV::instance3.isActive()) {
				UAV::instance3.setActive(false);
				UAV::instance1.setActive(true);
				Haptic::startPattern(Haptic::SingleShort);
			}
		}else if (Ui::instance.onButtonEvent(b, evt)) {
			break;
		}
